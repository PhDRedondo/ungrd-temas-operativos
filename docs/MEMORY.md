...
21. **FIC — captura AppSheet CONTROL FIC (sep 2026)** — `captureForms` (transferencia / legalización / prórroga), capa derivada de vigencia, lookup por No. CDP; plazos inicial+adición→final. KPI **Vencidos** = columna estado de legalización (`VENCIDO`), no cálculo por `fecha_final_para_legalizacion`. Geo persistida en DIVIPOLA (`normalizeRecordGeo` + `scripts/backfill-divipola-geo.ts`); filtros y bases cruzan el mismo catálogo. Excel `fields-from-source` intacto.
