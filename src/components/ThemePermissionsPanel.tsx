...
/** ACL de lectura/escritura por tema (contenido embebible). */
export function ThemePermissionsPanel() {
  const { role } = useAuth();
  ...
  if (!isAdmin) {
    return (
      <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-6 text-sm text-amber-950">
        Esta sección requiere rol <strong>admin</strong>.
      </div>
    );
  }
      ...
      <div className="flex flex-wrap items-end gap-3">
        <label className="block text-sm font-semibold text-ungrd-heading">
          ...
          >
            {users.length === 0 && (
              <option value="">
                (Aún no hay usuarios en BD — inicie sesión primero)
              </option>
            )}
            {users.map((u) => (
